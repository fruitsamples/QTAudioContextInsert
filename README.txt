...
------------------------
The QTAudioContextInsertApp application demonstrates the use of the QuickTime Audio Context Insert APIs which enable a client application to tap into and perform custom processing on QuickTime's audio stream during playback or movie audio extraction.
...
3. Select input/output layouts and underlying processing Audio Unit for the Audio Context Insert. This configures your insert.
4. Play the movie. The insert effect, selected in step (3) is applied to the movie audio being played back using the Audio Context Inserts APIs . 

[B] Extraction:
Follow steps 1 to 3 from the Playback section above
4. Open the Audio Extraction Window from the Window menu (Shift-Cmd-E).
5. Select the extraction channel layout, start time and duration, and then either preview the extraction or export to file. The insert that was configured in the Audio Context Insert Window in step (3) is applied to the audio stream being passed through movie audio extraction.